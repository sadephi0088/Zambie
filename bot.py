    ...
    try:
        ...
        for user_id in members:
            if not tagging:
                break
            try:
                # اگر کاربر username داشت از اون استفاده کن، در غیر اینصورت آیدی عددی
                user_mention = f"[{user_id}](tg://user?id={user_id})"
                msg = bot.send_message(tag_chat_id, f"👤 {user_mention} {tag_text}", parse_mode='Markdown')
...
# حذف لینک در پیام‌ها و کنترل قفل گروه
...
def check_links_and_locks(message):
    ...
    # ثبت یوزر در اعضا برای تگ
    group_members.add(uid)

# دستور /idd - نمایش اطلاعات کاربر (بدون عکس)
