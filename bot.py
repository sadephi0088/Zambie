...
if message.reply_to_message and is_admin(message.from_user.id):
try:
bot.restrict_chat_member(message.chat.id, message.reply_to_message.from_user.id, until_date=int(time.time()) + 60, can_send_messages=False)
...
break
try:
msg = bot.send_message(tag_chat_id, f"👤 {user_id} {tag_text}", parse_mode='Markdown')
...
except Exception:bot.reply_to(message, text)
else:
